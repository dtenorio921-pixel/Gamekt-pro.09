...
object GameFixesRegistry {
    ...
    fun applyFor(context: Context, appId: String, container: Container) {
        ...
        // Auto performance optimizer — applied before game-specific fixes so that
        // per-game fixes can still override individual settings if needed.
        if (source == GameSource.STEAM) {
            AutoGameOptimizer.applyForSteam(gameId, container, context)
        }
