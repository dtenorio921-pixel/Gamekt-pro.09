...
object PrefManager {
    ...
    // PERFORMANCE preset: enables BIGBLOCK=3, CALLRET=1, FORWARD=512, FASTNAN/FASTROUND=1
    // Gives better FPS for heavy games on ARM64 devices (e.g. Moto G35)
    private val BOX86_PRESET = stringPreferencesKey("box86_preset")
    var box86Preset: String
        get() = getPref(BOX86_PRESET, Box86_64Preset.PERFORMANCE)
        set(value) {
    ...
    var box64Preset: String
        get() = getPref(BOX64_PRESET, Box86_64Preset.PERFORMANCE)
        set(value) {
            setPref(BOX64_PRESET, value)
        }

    // FEXCore PERFORMANCE: disables TSO overhead, enables reduced precision — better FPS for heavy games
    ...
    // Default reduced from 2048 to 1024 MB for better compatibility with 4GB RAM devices (e.g. Moto G35)
    private val VIDEO_MEMORY_SIZE = stringPreferencesKey("videoMemorySize")
    var videoMemorySize: String
        get() = getPref(VIDEO_MEMORY_SIZE, "1024")
        set(value) {
